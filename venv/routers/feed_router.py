...
def update(feed_id: int, feed: FeedUpdate, db: Session=Depends(get_db), email: str=Depends(auth_service.get_current_user_authorization)):
    if email is None:
        raise HTTPException(status_code=401, detail="Not authorized") 
    return feed_service.get_update(db, feed_id, feed, email)

@router.delete("/delete/{feed_id}")
def delete(feed_id: int, db: Session=Depends(get_db), email: str=Depends(get_db)):
    if email is None:
        raise HTTPException(status_code=401, detail="Not authorized")
