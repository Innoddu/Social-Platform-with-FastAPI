...
def create_comment(comment: CommentCreate, db: Session=Depends(get_db), email: str=Depends(auth_service.get_current_user_authorization)):
...
def get_comments_by_feed_id(feed_id: int, db: Session=Depends(get_db)):
    return comment_service.get_comment_by_email(db, feed_id)
